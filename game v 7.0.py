...
class MyWindowClass(QtGui.QMainWindow, QtGui.QWidget, form_class):
    def __init__(self, parent=None):
        ...
        self.setupUi(self)
        chislo_Vse_Game_Name = len(Vse_Game_Name)
        for i in range(chislo_Vse_Game_Name):
            button_Vse_Game_Name = QtGui.QPushButton(Vse_Game_Name[i], self)
            button_Vse_Game_Name.setGeometry(100, (300+i*50), 100, 35)
            massiv_button_Vse_Game_Name.append(button_Vse_Game_Name)

            delete_Vse_Game_Name = QtGui.QPushButton(Vse_Game_Name[i], self)
            delete_Vse_Game_Name.setGeometry(450, (300+i*50), 100, 35)
            delete_Vse_Game_Name.setStyleSheet("background: red")
            massiv_delete_Vse_Game_Name.append(delete_Vse_Game_Name)
        for i in range(len(Vse_Game_Name)):
    ...
    def button_clicked0(self):
        ...
        if Game_Name not in Vse_Game_Name:
            Vse_Game_Name.append(Game_Name)
        with open('Vse_Game_Name.txt', 'w') as filehandle:  
            for listitem in Vse_Game_Name:
                filehandle.write('%s\n' % listitem)
...
Vse_Game_Name = []
with open("Vse_Game_Name.txt", 'r') as filehandle:
    for line in filehandle:
        currentPlace = line[:-1]
        Vse_Game_Name.append(currentPlace)
